...
docker_status = docker_data.get("status", "error")
redis_status = redis_data.get("status", "error")
...
d_images = docker_data.get("images", 0) or 0
d_volumes = docker_data.get("volumes", 0) or 0
d_projects = len(docker_data.get("compose_projects", [])) if docker_data.get("compose_projects") else 0
...
r_hit_rate = redis_data.get("hit_rate_percent", None)
